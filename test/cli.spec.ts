...
describe('list', () => {
  ...
  test('show more', async () => {
    ...
    expect(stdout).toBe(
      Logger.grid(
        list.map(([name, item]) => {
          return [name, inspect(item, { colors: true, compact: false, depth: 1 })]
        })
      )
    )
  })
...
describe('add', () => {
  test('no argument', async () => {
    ...
    expect(stdout).toBe(Logger.error('missing required args for command `add <repo>`'))
  })

  test('invalid repo', async () => {
    const stdout = await cli('add', ['foo/bar?foo=bar'])
    expect(stdout).toBe(Logger.error(`Could not parse the input: 'foo/bar?foo=bar'.`))
  })

  test('no token', async () => {
    const stdout = await cli('add', [repoForTest.input])
    expect(stdout).toBe(Logger.error('GitHub personal access token is not configured.'))
  })
})
...
describe('add with token(repo scope)', () => {
  ...
  test('with depth 1', async () => {
    ...
    expect(stdout).toBe(Logger.grid(['a', 'b', 'c'].map((val) => [`+ ${val}`, ''])))
    const dirs = await cacheController.readdir()
    expect(dirs).toHaveLength(1)
    const local = dirs[0]
    const storeContent = await store.getContent()
    ...
    const comparable = { input: repoForTest.input, url, sha: shaOfDefaultBranch }
    expect(a).toStrictEqual({ ...comparable, local: cacheController.join(local, 'a') })
    expect(b).toStrictEqual({ ...comparable, local: cacheController.join(local, 'b') })
    expect(c).toStrictEqual({ ...comparable, local: cacheController.join(local, 'c') })
  })
  ...
  test('with subdir and depth 1', async () => {
    ...
    expect(stdout).toBe(Logger.grid(['a1', 'a2', 'a3'].map((val) => [`+ ${val}`, ''])))
    const dirs = await cacheController.readdir()
    expect(dirs).toHaveLength(1)
    const local = dirs[0]
    const storeContent = await store.getContent()
